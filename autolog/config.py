...
def read_data(file_path: str, credentials_user_input: bool):
    ...
    if v.validate(data, schema):  # type: ignore
        ...
        for index, autolog_instance in enumerate(autolog):
            ...
            if not trigger.get("trigger_pv_value") and not trigger.get("on_change"):
                logging.error(
                    "On Autolog %s: You should provide at least 'trigger_pv_value' "
                    "or 'on_change' "
                ...
                logging.error(
                    "On Autolog %s: You should provide 'trigger_pv_value' "
                    "OR 'on_change' "
                    "into trigger section of configuration file",
                    index,
                )
    ...
    logging.error(
        "Configuration is invalid, please refer to README to know the parameters"
    )
    sys.exit()
